...
import subprocess
# Version: 1.4 RemiVpn
# Update source: edit UPDATE_SOURCE_URL below to point at your own repo
LICENSE_USER = "Sin.Kabir"
LICENSE_REMAINING = "∞"
LICENSE_PLAN = "g"
UPDATE_SOURCE_URL = "https://raw.githubusercontent.com/Joud-prog/RemiVpn/main/install.sh"
...
INSTANCES_DIR = "/etc/tor/remivpn_instances"
DATA_DIR_BASE = "/var/lib/tor/remivpn_data"
INSTALL_FLAG_FILE = "/etc/tor/remivpn_installed"
CONFIG_MODE_FILE = "/etc/tor/remivpn_config_mode"
...
def _banner_line(text, color=None):
    content = text.center(BANNER_WIDTH)
    if color:
        content = f"{color}{content}{COLOR_RESET}"
    return f"{COLOR_PRIMARY} │{COLOR_RESET}{content}{COLOR_PRIMARY}│{COLOR_RESET}"

def print_banner():

    print(f"{COLOR_PRIMARY} ┌────────────────────────────────────────────────────────┐{COLOR_RESET}")
    print(_banner_line(""))
    print(_banner_line("R E M I V P N", COLOR_SECONDARY))
    print(_banner_line("Multi-Node VPN Manager", COLOR_MUTED))
    print(_banner_line(""))
    print(f"{COLOR_PRIMARY} ├────────────────────────────────────────────────────────┤{COLOR_RESET}")
...
def is_valid_port(port_str):
    """Validate a user-supplied port string is a real TCP port (1-65535)."""
    if not port_str or not port_str.isdigit():
        return False
    return 1 <= int(port_str) <= 65535

def allocate_ports(country_code, mode, installed):
    """Single source of truth for SOCKS/Control port allocation.

    Used by single-node install, bulk install (auto + manual), and silent
    deploy so the allocation rule can never drift between call sites.
    """
    if mode == 'static':
        socks, control = get_static_port_for_country(country_code)
        while not is_port_free(socks) or socks > 65535:
            socks += 1
            control = socks + 10000
    else:
        used_ports = {int(inst['socks']) for inst in installed.values() if inst['socks'] and inst['socks'].isdigit()}
        candidate = BASE_PORT
        while candidate in used_ports or not is_port_free(candidate):
            candidate += 1
        socks = candidate
        control = socks + 10000
    return socks, control
...
def get_installed_instances():
    ...
    if os.path.exists(INSTANCES_DIR):
        for item in os.listdir(INSTANCES_DIR):
            if item.startswith("remivpn_"):
                all_codes.add(item.replace("remivpn_", ""))
    ...
    for country_code in all_codes:
        ...
        service_file = f"/etc/systemd/system/remivpn-{country_code}.service"
        active_path = None
        if os.path.exists(service_file):
            try:
                with open(service_file, 'r') as sf:
                    content = sf.read()
                    if "remivpn_instances" in content:
                        active_path = f"{INSTANCES_DIR}/remivpn_{country_code}"
                    elif "/etc/tor/instances" in content:
                        active_path = f"{OLD_INSTANCES_DIR}/remivpn_{country_code}"
            except Exception:
                pass
        ...
        if active_path and os.path.exists(f"{active_path}/torrc"):
            ...
            new_path = f"{INSTANCES_DIR}/remivpn_{country_code}"
            old_path = f"{OLD_INSTANCES_DIR}/remivpn_{country_code}"
...
def delete_node(country_code):
    ...
    run_cmd(["sudo", "systemctl", "stop", f"remivpn-{country_code}"])
    run_cmd(["sudo", "systemctl", "disable", f"remivpn-{country_code}"])
    ...
    config_dir = f"{INSTANCES_DIR}/remivpn_{country_code}"
    data_dir = f"{DATA_DIR_BASE}/remivpn_{country_code}"
    ...
    old_config_dir = f"{OLD_INSTANCES_DIR}/remivpn_{country_code}"
    old_data_dir = f"{OLD_DATA_DIR_BASE}/remivpn_{country_code}"
...
def create_remivpn_instance(country_code, socks_port, control_port):

    instance_name = f"remivpn_{country_code}"
    ...
    config_content = f"""# RemiVpn configuration for {country_code.upper()}
...
"""
    ...
    perms_ok = True
    perms_ok &= run_cmd(["sudo", "chown", "-R", "debian-tor:debian-tor", inst_conf_dir])
    perms_ok &= run_cmd(["sudo", "chown", "-R", "debian-tor:debian-tor", inst_data_dir])
    perms_ok &= run_cmd(["sudo", "chmod", "700", inst_data_dir])
    if not perms_ok:
        print(f"{COLOR_ERROR}[-] Failed to set ownership/permissions on {inst_data_dir} — Tor would not be able to start as debian-tor.{COLOR_RESET}")
        return False

    service_file = f"/etc/systemd/system/remivpn-{country_code}.service"
    service_content = f"""[Unit]
Description=RemiVpn Connection Node for {country_code.upper()}
...
"""
    ...
    run_cmd(["sudo", "systemctl", "daemon-reload"])
    run_cmd(["sudo", "systemctl", "enable", f"remivpn-{country_code}"])
    started = run_cmd(["sudo", "systemctl", "restart", f"remivpn-{country_code}"])
    if not started:
        print(f"{COLOR_ERROR}[-] systemctl failed to start remivpn-{country_code} — check 'journalctl -u remivpn-{country_code}'.{COLOR_RESET}")
        return False
...
def perform_smart_bootstrap(country_code, socks_port):
    ...
    service_name = f"remivpn-{country_code}"
...
def setup_additional_location():
    ...
    while True:
        ...
        mode = get_port_mode()
        next_socks_port, next_control_port = allocate_ports(selected_code, mode, installed)
        ...
        if create_remivpn_instance(selected_code, next_socks_port, next_control_port):
...
def view_installed_locations():
    ...
    for idx, (code, info) in enumerate(sorted_instances, 1):
        ...
        status_check = subprocess.run(['systemctl', 'is-active', f'remivpn-{code}'], stdout=subprocess.PIPE, text=True)
        ...
        while True:
            ...
            if act_choice == '0' or not act_choice:
                ...
                if not is_valid_port(new_port):
                    print(f"{COLOR_ERROR}[- ] Error: Port must be a number between 1 and 65535.{COLOR_RESET}")
                    time.sleep(1.5)
                    continue
                ...
                if int(new_port) + 10000 > 65535:
                    print(f"{COLOR_ERROR}[- ] Error: Port too high — control port ({new_port}+10000) would exceed 65535.{COLOR_RESET}")
                    time.sleep(2)
                    continue

                new_control = int(new_port) + 10000
                print(f"[*] Reconfiguring node {target_name} on port {new_port}...")
                create_remivpn_instance(target_code, new_port, new_control)
...
def uninstall_system():
    ...
    if not is_system_installed():
        print(f"{COLOR_WARN}[!] RemiVpn system is already uninstalled or not detected on this server.{COLOR_RESET}")
        input("\nPress Enter to return to main menu...")
        return
        
    confirm = input("⚠️ Are you sure you want to completely uninstall RemiVpn and wipe all configurations? [y/N]: ").strip().lower()
    if confirm != 'y':
    ...
    print(f"\n{COLOR_SUCCESS}[+] RemiVpn system successfully completely uninstalled and resources reclaimed!{COLOR_RESET}")
    input("\nPress Enter to return to main menu...")
...
def update_system():
    ...
    print(f"{COLOR_WARN}[!] This will download and run a shell script from:{COLOR_RESET}\n    {UPDATE_SOURCE_URL}")
    ...
    if confirm in ['y', 'yes']:
        ...
        time.sleep(1.5)
        cmd = f'curl -fsSL "{UPDATE_SOURCE_URL}?v=$(date +%s)" | tr -d "\\r" | sudo bash'
        rc = subprocess.run(["bash", "-c", cmd]).returncode
        if rc != 0:
            print(f"{COLOR_ERROR}[-] Update script exited with an error (code {rc}). Nothing was changed if the download failed.{COLOR_RESET}")
            input("\nPress Enter to return...")
            return
        sys.exit(0)
    else:
...
def deploy_node_silently(code, name, mode, installed):
    ...
    socks, control = allocate_ports(code, mode, installed)

    if create_remivpn_instance(code, socks, control):
        verified, live_ip = perform_smart_bootstrap(code, socks)
        if verified:
            installed[code] = {'socks': str(socks), 'control': str(control), 'path': f"{INSTANCES_DIR}/remivpn_{code}"}
            return True, live_ip
        else:
            delete_node(code)
            return False, None
...
def launch_ez_panel():
    ...
    if installed_path:
        ...
        print(f"{COLOR_WARN}[!] Fetching install script from https://raw.githubusercontent.com/1NoJoom/EZ-Panel/main/install.sh{COLOR_RESET}")
        time.sleep(1)
        cmd = 'sudo bash -c "$(curl -fsSL "https://raw.githubusercontent.com/1NoJoom/EZ-Panel/main/install.sh?v=$(date +%s)" | tr -d \'\\r\')"'
        rc = subprocess.run(["bash", "-c", cmd]).returncode
        if rc != 0:
            print(f"{COLOR_ERROR}[-] EZ-Panel install script failed (code {rc}).{COLOR_RESET}")
            input("\nPress Enter to return...")
            return
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            clear_screen()
            print(f"{COLOR_SUCCESS}Thank you for using RemiVpn Multi-Node Master. Exiting...{COLOR_RESET}")
            break
        else:
            print(f"{COLOR_ERROR}[-] Invalid selection.{COLOR_RESET}")
            time.sleep(1.5)
